...
const CollectionCard = ({
  ...
  useEffect(() => {
    Auth.getByWallet(item.collection_owner)
      .then((res) => {
        if (res) {
  ...
  return (
    ...
    >
      <Link to={`/collection/${item._id}`}>
        <div
          css={{
            backgroundImage: `linear-gradient(180deg, rgba(0, 0, 0, 0.3) 0%, rgba(0, 0, 0, 0.179474) 29.92%, rgba(0, 0, 0, 0.189) 73.96%, rgba(0, 0, 0, 0.3) 100%), url(${item.collection_background_image_url})`,
      ...
      >
        <div tw="flex items-center gap-2.5 overflow-hidden">
          <div tw="overflow-hidden">
            <Link to={`/collection/${item._id}`} tw="overflow-hidden">
