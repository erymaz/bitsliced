...
import { Channeloffer, Collection, Order } from '../../api/api';
...
const PurchasePopupTitle: { [key: number]: string } = {
  [PurchaseStep.CONFIRM]: 'Confirm Channel Offer',
  [PurchaseStep.ACCEPT]: 'Accept offer',
};

const AcceptChannelOfferPopup = (props: {
  channel?: Partial<ChannelData>;
  buyer?: Partial<User>;
  offer: ChannelOfferData | null;
  onClose: () => void;
}) => {
  ...
  const accepting = async () => {
    ...
    console.log('channeloffer: ', channeloffer);
    ...
    contract
      .sellChannel(channeloffer, sig.v, sig.r, sig.s)
      .then(async (res: any) => {
        if (res.hash) {
          // call api put method : order {transactionHash: res.hash, transactionStatus: 0}
          if (props.offer?._id) {
            Channeloffer.update(props.offer._id, {
              seller: account || '',
              ...
              .then((updateRes) => {
                if (updateRes) {
                  timer = setInterval(() => {
                    if (props.offer?._id) {
                      Channeloffer.getById(props.offer._id).then(
                        (getRes: ChannelofferParam) => {
                          if (
                            getRes &&
                            ...
                            if (
                              ...
                              alertSuccess(
                                'You accepted the channel offer successfully.'
                              );
                              setLoading(false);
                            } else {
                              setLoading(false);
...
export default AcceptChannelOfferPopup;
