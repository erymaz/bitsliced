...
import { Auth, Ticket } from '../../api/api';
...
const RenewChannelTicketPopup = (props: {
  channel?: ChannelData;
  ticket?: TicketData;
  onClose: () => void;
}) => {
  let { account, active, library } = useWeb3React();
  ...
  useEffect(() => {
    if (props.channel) {
      Auth.getByWallet(props.channel?.channel_creator).then((res) =>
        setCreator(res)
      );
    }
  });
  ...
  const handleExtendChannelTicket = async () => {
  ...
  const extendChannelTicket = async () => {
    let timer: number | NodeJS.Timer | null;

    if (props.channel?._id && props.channel?.channel_address && props.ticket) {
      setWaitingTransaction(2);
      const ticket_id = props.ticket._id ?? '';
      const ticketId = props.ticket.ticketId ?? ''; //*************************************** Please update here */
      if (active) {
        try {
          const contract = new ethers.Contract(
            props.channel?.channel_address || '',
                ...
                Ticket.update(ticket_id, {
                  ...
                  .then((uptRes) => {
                    if (uptRes) {
                      ...
                      timer = setInterval(() => {
                        Ticket.getById(ticket_id).then((getRes: TicketData) => {
                          if (
                            getRes &&
                            getRes.status !== TransactionStatus.PENDING
                          ) {
                            if (getRes.status === TransactionStatus.CONFIRMED) {
                              props.onClose();
                              alertSuccess('A ticket has been extended.');
                              setTimeout(() => {
          ...
          <div tw="w-full md:w-[41%] px-3.5 pt-[22px] pb-3.5 bg-light/50 dark:bg-light/5 rounded-lg">
            ...
            <p tw="pt-8 text-base leading-[150%] tracking-tight text-center text-gray-500">
              ...
              <strong tw="font-semibold text-base leading-[150%] tracking-tight text-dark dark:text-light">
                {parseInt(selectedOptions[1]) *
                  (props.channel?.channel_ticket_price ?? 0)}
              </strong>{' '}
            ...
            <div tw="pt-6 w-full">
              <button
                ...
                onClick={() => handleExtendChannelTicket()}
              >
...
export default RenewChannelTicketPopup;
