...
const PurchasePopupTitle: { [key: number]: string } = {
  [MakeOfferStep.REVIEW]: 'This is an unreviewed Channel Ticket',
  [MakeOfferStep.INPUT]: 'Make a Channel Ticket offer',
  [MakeOfferStep.COMPLETE]: 'Channel Ticket offer is complete',
};

const MakeTicketOfferPopup = (props: {
  item?: Partial<TicketData>;
  onClose: () => void;
}) => {
  ...
  useEffect(() => {
    if (props.item?.channel_id) {
      Channel.getById(props.item.channel_id).then((res) => {
        if (res) {
          setChannel(res);
          if (res.channel_creator) {
            Auth.getByWallet(res.channel_creator).then((res) =>
              setCreator(res)
            );
          }
        }
    ...
    if (props.item?.channel_id && props.item.ticketId) {
    ...
    if (props.item?.owner) {
      Auth.getByWallet(props.item.owner).then((res) => setOwner(res));
    }
  }, [props.item]);
    ...
    contract
      ...
      .then(async (res: any) => {
        if (res.hash) {
          ...
          localStorage.setItem('txHashForMakeOfferTicket', res.hash);
        }
      })
      .catch((e: any) => {
        console.error(e);
  ...
  useEffect(() => {
    const timer1 = setInterval(async () => {
      const txHash = localStorage.getItem('txHashForMakeOfferTicket') || '';
      ...
      if (txHash) {
        ...
        try {
          ...
          if (res && res.confirmations > 1) {
            localStorage.removeItem('txHashForMakeOfferTicket');
            locked = false;
            setStep(MakeOfferStep.SIGN);
          }
          locked = false;
    ...
    if (offerAmount && offerAmount > 0) {
      try {
        ...
        const value = {
          ...
          tokenId: Number(props.item?.ticketId || ''),
        };
        ...
        Ticketoffer.create({
          ...
          .finally(() => {
            console.log('final... POST Ticketoffer.create');
          });
      } catch (e: any) {
        console.error(e);
        alertError(e.toString());
          ...
          <div tw="mx-auto p-6 w-full max-w-[489px] flex justify-center shadow-[0px 4px 14px rgba(0, 0, 0, 0.1)] rounded-lg">
            <div tw="w-full max-w-[520px] flex flex-col gap-6">
              ...
              <TextLine
                ...
                text={
                  <Link to={`/channels/${channel?._id}`}>
                    {shortAddress(channel?.channel_address ?? '')}
                  </Link>
                }
                title={channel?.channel_address ?? ''}
              />
              <TextLine
                label="Channel creator"
                text={
                  <CopyToClipboard
                    text={channel?.channel_creator ?? ''}
                    onCopy={() => alertInfo('Copied.')}
                  >
                    <div tw="flex items-center gap-1 cursor-pointer">
                      <Avatar pic={creator?.profile_image_url} size={40} />
                      {creator?.name}
                    </div>
                  </CopyToClipboard>
                }
                title={channel?.channel_creator}
              />
              <TextLine label="Ticket Id" text={props.item.ticketId} />
              <TextLine label="Channel ticket price" text={props.item.price} />
              <TextLine
                label="Best offer for this ticket"
                text={bestOffer === -Infinity ? '-' : bestOffer}
              />
              <TextLine
                label="Owner of the ticket"
                text={
                  <CopyToClipboard
                    text={props.item.owner ?? ''}
                    onCopy={() => alertInfo('Copied.')}
                  >
                    <div tw="flex items-center gap-1 cursor-pointer">
                      <Avatar pic={owner?.profile_image_url} size={40} />
                      {owner?.name}
                    </div>
                  </CopyToClipboard>
                }
                title={shortAddress(props.item.owner ?? '')}
              />
              <TextLine
                label="Ticket creation date"
                text={
                  props.item?.createdAt
                    ? format(new Date(props.item.createdAt), 'M/d/yyyy, h:mm a')
                    : '-'
                }
              />
              <TextLine
                label="Ticket expiration date"
                text={
                  props.item?.expiredTimestamp
                    ? format(
                        new Date(props.item.expiredTimestamp),
                        'M/d/yyyy, h:mm a'
                      )
                    : '-'
                }
          ...
          <div tw="p-[20px] relative w-full">
            ...
            <div tw="pb-[30px] flex justify-between items-center">
              ...
              <div tw="flex flex-col items-end">
                <div>No. {props.item.ticketId || ''}</div>
                <div>{props.item?.price} SLICED</div>
              </div>
            </div>
            ...
            <div tw="px-6 py-6 w-full max-w-[560px] flex flex-col md:flex-row justify-center items-center gap-3 bg-white dark:bg-dark rounded-md shadow-lg">
              ...
              <div>
                <p tw="font-normal text-base tracking-tight leading-[140%] text-center text-dark/70 dark:text-light/70">
                  Approving SLICED token for making ticket offer, please accept
        ...
        {step === MakeOfferStep.SIGN ? (
          <div tw="fixed left-0 top-0 w-full h-full flex justify-center items-center z-50 px-4">
            <div tw="px-6 py-6 w-full max-w-[560px] flex flex-col md:flex-row justify-center items-center gap-3 bg-white dark:bg-dark rounded-md shadow-lg">
              ...
              <div>
                <p tw="font-normal text-base tracking-tight leading-[140%] text-center text-dark/50 dark:text-light/50">
                  Confirming the ticket offer, please accept the following
...
export default MakeTicketOfferPopup;
