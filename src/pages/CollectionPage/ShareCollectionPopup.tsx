...
import { ChannelData, ChannelPermission, CollectionData } from '../../type.d';
...
const ShareCollectionPopup = (props: {
  ...
  useEffect(() => {
    if (props.collection) {
      setMimeType(getMimeType(props.collection.collection_profile_image_url));
    }
  }, [props.collection]);
  ...
  useEffect(() => {
    if (step === SharingStep.PENDING && user) {
      Posting.create({
        ...
        channelpost_shared_collection_id: props.collection._id,
        user_id: user._id ?? '',
      })
        .then((res) => {
          if (res) {
            alertSuccess(
              `This collection and your thought have been published on the channel "${channel?.channel_name}".`
            );
          }
        })
        .catch((e) => {
  ...
  }, [step, user, props.collection, channel, thought]);
  ...
  const sharedUrl = useMemo(
    () =>
      window.location
        ? `${window.location.protocol}//${window.location.host}/collection/${props.collection?._id}`
        : `http://18.205.237.239/collection/${props.collection?._id}`,
    [props.collection]
  );
      ...
      >
        ...
        <h3 tw="font-bold text-[23px] tracking-tight leading-[150%] text-center text-[rgba(0, 0, 0, 0.9)] dark:text-[rgba(255, 255, 255, 0.9)]">
          {step === SharingStep.INITIAL && (
            <>Share {props.collection.collection_name}</>
          )}
          {step === SharingStep.PENDING && (
            <>Sharing {props.collection.collection_name}...</>
          )}
        ...
        <div tw="pt-[40px] pb-[30px] flex items-center gap-[10px]">
          <div
            css={{
              backgroundImage: mimeType.startsWith('video')
                ? 'none'
                : `url(${props.collection.collection_profile_image_url})`,
            }}
          ...
          >
            {mimeType.startsWith('video') && (
              ...
              >
                <source
                  src={props.collection.collection_profile_image_url}
                  type={mimeType}
                />
              </video>
            )}
                ...
                onClick={() => {
                  if (channel) {
                    if (sharable) {
                      ...
                      alertWarning(
                        'This channel is not allowed to share the collections in it.'
                      );
                    }
                  } else {
            ...
            <>
              ...
              >
                <span tw="font-semibold text-base tracking-tight text-dark dark:text-light ">
                  List {props.collection.collection_name}
                </span>
        ...
        <div tw="pt-[30px] border-t-2 border-[rgba(0, 0, 0, 0.15)] dark:border-[rgba(255, 255, 255, 0.15)]">
          ...
          <div tw="flex items-center gap-[20px]">
            ...
            <a
              href={`https://twitter.com/intent/tweet?text=Explore the latest ${props.collection.collection_name} on the Sliced marketplace today!%0aClick the link below, and find the perfect addition to your collection.%0a${sharedUrl}`}
              rel="noreferrer"
              target="_blank"
            ...
            <a
              href={`https://telegram.me/share/url?url=${sharedUrl}&text=${encodeURI(
                `Explore the latest ${props.collection.collection_name} on the Sliced marketplace today!\nClick the link below, and find the perfect addition to your collection.\n`
              )}${sharedUrl}`}
              rel="noreferrer"
              target="_blank"
              tw="flex items-center gap-[4px]"
...
export default ShareCollectionPopup;
