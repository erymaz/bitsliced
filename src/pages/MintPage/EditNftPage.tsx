...
const EditNftPage = () => {
  ...
  useEffect(() => {
    if (params.nft) {
      Nft.getById(params.nft)
        ...
        .catch((e) => {
          alertError('Cannot get the NFT data.');
        });
    }
  }, [params.nft]);
  ...
  const submitNftData = () => {
    if (!nft) {
      alertError('Cannot load the NFT data.');
      return;
    }
    ...
    if (nft?._id && !nft?.isMinted) {
      ...
      alertWarning('Cannot edit the minted NFT.');
    }
  };
  ...
  return (
    <div tw="mx-auto px-4 py-8 w-full max-w-[1392px]">
      ...
      <div tw="mx-auto pt-8 w-full max-w-[755px]">
        <StyledPageTitle>Edit "{nft?.name}"</StyledPageTitle>
