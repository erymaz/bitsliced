...
import { ChannelData, ChannelPermission } from '../../type.d';
...
const ShareChannelPopup = (props: {
  ...
  useEffect(() => {
    if (step === SharingStep.PENDING && user) {
      Posting.create({
        ...
        channelpost_shared_channel_id: props.channel._id,
        user_id: user._id ?? '',
      })
        .then((res) => {
          if (res) {
            alertSuccess(
              `This channel and your thought have been published on the channel "${channel?.channel_name}".`
            );
          }
        })
        .catch((e) => {
  ...
  }, [step, user, props.channel, channel, thought]);
  ...
  const sharedUrl = useMemo(
    () =>
      window.location
        ? `${window.location.protocol}//${window.location.host}/channels/${props.channel?._id}`
        : `http://18.205.237.239/collection/${props.channel?._id}`,
    [props.channel]
  );
      ...
      >
        ...
        <h3 tw="font-bold text-[23px] tracking-tight leading-[150%] text-center text-[rgba(0, 0, 0, 0.9)] dark:text-[rgba(255, 255, 255, 0.9)]">
          {step === SharingStep.INITIAL && (
            <>Share {props.channel.channel_name}</>
          )}
          {step === SharingStep.PENDING && (
            <>Sharing {props.channel.channel_name}...</>
          )}
                ...
                onClick={() => {
                  if (channel) {
                    if (sharable) {
                      ...
                      alertWarning(
                        'This channel is not allowed to share the channels in it.'
                      );
                    }
                  } else {
        ...
        <div tw="pt-[30px] border-t-2 border-[rgba(0, 0, 0, 0.15)] dark:border-[rgba(255, 255, 255, 0.15)]">
          ...
          <div tw="flex items-center gap-[20px]">
            ...
            <a
              href={`https://twitter.com/intent/tweet?text=Join the Channel ${props.channel.channel_name} on the Sliced marketplace today!%0aClick the link below, and start expanding your network and knowledge.%0a${sharedUrl}`}
              rel="noreferrer"
              target="_blank"
            ...
            <a
              href={`https://telegram.me/share/url?url=${sharedUrl}&text=${encodeURI(
                `Join the Channel ${props.channel.channel_name} on the Sliced marketplace today!\nClick the link below, and start expanding your network and knowledge.\n`
              )}${sharedUrl}`}
              rel="noreferrer"
              target="_blank"
              tw="flex items-center gap-[4px]"
...
export default ShareChannelPopup;
